...
function ProductCard({ productDsc, productImgUrl }) {
    ...
    const hoverCard = (status) => {
        ...
        if (status === true) {
            setCaptionStyle("card__dsc  absolute bottom-0 left-0 text-white rounded-2xl p-5 flex flex-row justify-start items-end  mt-20 z-50  text-2xl font-bold text-my_dark_gray ");
            ...
            setCaptionStyle("card__dsc text-white rounded-2xl absolute bottom-0 left-0 p-5 flex flex-row justify-start items-end  mt-20 z-50  text-2xl font-bold text-my_white");
